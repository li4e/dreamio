...
export class CloudStorage implements ICloudStorage {
  async saveGenerationImageFromBase64(): Promise<ICloudStorageItem> {
    return {
      delete: async () => {
        //
      },
  ...
  async saveUserAvatar(userId: number, tempFilePath: string) {
    return {
      delete: async () => {
        //
      },
      imageData: {
        filePath: `users/${userId}/avatars/${uuidv4()}.jpg`,
        publicUrl:
          'https://thumb.photo-ac.com/7a/7a706ac91fa3a2330382e7f4c11c6b3e_t.jpeg',
      },
    }
